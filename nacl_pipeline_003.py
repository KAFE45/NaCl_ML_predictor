...
def prepare_dataset(input_file: Path) -> tuple:
    ...
    # โหลดครั้งเดียว และลบ is_simulated ทันที
    df = pd.read_csv(input_file)
    df = df.drop(columns=["is_simulated"], errors="ignore")
...
def visualise(y_test, y_pred, metrics: dict,
    ...
    plt.suptitle("Random Forest — NaCl Concentration Predictor 003 ",
                 fontsize=14, fontweight="bold", y=1.01)

    plt.tight_layout()

    plot_path = out_dir / f"rf_results_003_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.png"
...
def calibrate_k_nacl_poly(input_file: Path) -> tuple:
    """
    Fit polynomial curve: EC_25C = f(NaCl)
    แทนสมการเส้นตรงที่ไม่ fit กับ sensor นี้
    คืนค่า: (poly_model, r2_score)
    """
    ...
    print(f"\n  📊 Polynomial Fit (degree=2): EC_25C = f(NaCl)")
    print(f"  R²  = {r2:.4f}  {'✅ ผ่าน' if r2 >= 0.95 else '❌ ไม่ผ่าน'}")
    print(f"  MAE = {mae:.2f}")

    # Plot
    nacl_range = np.linspace(0, 2.5, 100).reshape(-1, 1)
    ec_pred    = poly_model.predict(nacl_range)

    plt.figure(figsize=(8, 5))
    plt.scatter(df["NaCl_Percent"], df["EC_25C"],
                alpha=0.3, s=20, label="Real Data")
    plt.plot(nacl_range, ec_pred,
             color="red", linewidth=2, label=f"Poly fit (R²={r2:.3f})")
    plt.xlabel("NaCl %")
    plt.ylabel("EC_25C")
    plt.title("EC_25C vs NaCl — Polynomial Fit")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("ec_nacl_poly_fit.png", dpi=150)
    plt.show()
    print(f"  ✔ บันทึกกราฟที่ ec_nacl_poly_fit.png")

    return poly_model, r2


def simulate_nacl_data(poly_model,
                       nacl_range: tuple,
                       temp_range: tuple,
                       n_points: int = 300,
                       random_seed: int = 42) -> pd.DataFrame:
    """สร้างข้อมูลจำลองโดยใช้ poly_model ที่ validate แล้ว"""
    ALPHA = 0.02
    ...
    # ทำนาย EC_25C จาก NaCl ด้วย poly_model
    ec_25c_pred = poly_model.predict(nacl.reshape(-1, 1))

    # แปลง EC_25C → EC ที่ temp จริง
    ec_true  = ec_25c_pred * (1 + ALPHA * (temp_c - 25))
...
def find_true_alpha(input_file: Path) -> float:
    """
    หาค่า α ที่แท้จริงของ sensor นี้
    โดย optimize α ให้ EC_25C มี correlation สูงสุดกับ NaCl_Percent
    """
    ...
    def neg_r2(alpha):
        ec_25c = df["EC_M02"] / (1 + alpha * (df["Mercury_Temp"] - 25))
        correlation = np.corrcoef(ec_25c, df["NaCl_Percent"])[0, 1]
        return -correlation  # minimize negative = maximize correlation

    result = minimize_scalar(neg_r2, bounds=(0.001, 0.10), method="bounded")
    best_alpha = result.x
    best_r = -result.fun

    print(f"\n  📊 Alpha Optimization:")
    print(f"  IEC standard α = 0.0200  (assumed)")
    print(f"  Sensor true  α = {best_alpha:.4f}  (optimized)")
    print(f"  Best correlation = {best_r:.4f}")

    # เปรียบเทียบ R² ก่อน/หลัง
    ec_std  = df["EC_M02"] / (1 + 0.02   * (df["Mercury_Temp"] - 25))
    ec_opt  = df["EC_M02"] / (1 + best_alpha * (df["Mercury_Temp"] - 25))

    r2_std = r2_score(df["NaCl_Percent"],
                      ec_std / ec_std.mean() * df["NaCl_Percent"].mean())
    r2_opt = r2_score(df["NaCl_Percent"],
                      ec_opt / ec_opt.mean() * df["NaCl_Percent"].mean())

    print(f"  R² with α=0.02       : {r2_std:.4f}")
    print(f"  R² with α={best_alpha:.4f}  : {r2_opt:.4f}")
